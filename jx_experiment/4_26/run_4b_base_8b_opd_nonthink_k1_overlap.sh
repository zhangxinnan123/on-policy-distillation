...
# Coverage-based Hybrid Masking Strategy
...
HYBRID_MASK_COVERAGE_THRESHOLD=0.1  # Coverage threshold for PG vs supervised routing  
...
EXP_NAME="fsdp/student-${STUDENT_MODEL}/teacher-${TEACHER_MODEL}/loss-${DISTILLATION_LOSS_MODE}/mask-coverage_p${HYBRID_MASK_TOP_P}_t${HYBRID_MASK_COVERAGE_THRESHOLD}_pg${PG_LOSS_COEF}_sup${SUPERVISED_LOSS_COEF}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}_nonthink150"
...
DISTILLATION=(
    ...
    distillation.distillation_loss.topk=8
    ...
    +distillation.distillation_loss.hybrid_mask_kwargs.top_p=$HYBRID_MASK_TOP_P
    +distillation.distillation_loss.hybrid_mask_kwargs.coverage_threshold=$HYBRID_MASK_COVERAGE_THRESHOLD
    +distillation.distillation_loss.hybrid_mask_kwargs.inverse=$HYBRID_MASK_INVERSE
)
...
echo "================== Coverage-based Hybrid Mask Configuration =================="
echo "HYBRID_MASK_STRATEGY: $HYBRID_MASK_STRATEGY"
echo "HYBRID_MASK_TOP_P: $HYBRID_MASK_TOP_P"
echo "HYBRID_MASK_COVERAGE_THRESHOLD: $HYBRID_MASK_COVERAGE_THRESHOLD"
echo "HYBRID_MASK_INVERSE: $HYBRID_MASK_INVERSE"

...
HYBRID_MASK_STRATEGY="opd_theory_guided"
...
EXP_NAME="fsdp/student-${STUDENT_MODEL}/teacher-${TEACHER_MODEL}/loss-${DISTILLATION_LOSS_MODE}/mask-opdt_p${HYBRID_MASK_TEACHER_TOP_P}_eps${HYBRID_MASK_ADV_EPS}_pg${PG_LOSS_COEF}_sup${SUPERVISED_LOSS_COEF}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}_partial4_subeos_hicov"

ENFORCE_EAGER=True # true for faster debugging

############################ Paths ############################
DATA_PATH="${HOME}/data/dapo_17k_aime2426-suffix-partial4"
...
DATA=(
    ...
    +data.sampler.class_path=pkg://verl.experimental.dataset.partial_curriculum_sampler
    +data.sampler.class_name=PartialCurriculumSampler
    +data.sampler.stages=[3,2,1,0]
    +data.sampler.within_stage=shuffle
    +data.sampler.seed=42
    +data.sampler.steps_per_stage=60
)

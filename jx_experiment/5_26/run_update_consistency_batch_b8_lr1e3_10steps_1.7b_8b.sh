#!/usr/bin/env bash
# Update-consistency — BATCH MODE (all tokens contribute to gradient) — B=8, lr=1e-3.
# Compare against B=1 single-token anchor:
#   run_update_consistency_singletoken_b1_lr1e3_50steps_1.7b_8b.sh
# Same lr / loss / response_length so per-row stats are comparable.
#
# Key differences vs B=1 single-token:
#   - single_token_mode=false         → every (i, j) in response_mask contributes
#                                       (the realistic on-policy distillation update)
#   - TRAIN_PROMPT_BSZ=8              → 8 sequences × ~50 valid tokens = ~400 grad
#                                       contributors per step (vs 1 in B=1)
#   - STUDENT_WORLD_SIZE=4            → dp_size=4 (vs 1)
#   - 10 steps (quick look)           → ~400 × 10 = 4000 token-positions of stats
#
# What this run is for:
#   Test whether the per-token sign rule survives batched gradients. In the B=1
#   single-token anchor, sampled-u sign agreement hit 100% (|A|>0.1). In batch,
#   parameter sharing means each token's update is contaminated by ALL OTHER
#   tokens' gradients in the same micro-batch via the shared backbone, so the
#   per-token theory may break.
#
# Diff vs jx_experiment/5_25/run_update_consistency_sgd_batch_lr1e3_1.7b_8b.sh:
#   only TOTAL_TRAINING_STEPS bumped from 5 to 10.
...
MEASURE_EVERY=1
OPTIMIZER=SGD
LR=1e-3  # 100x baseline; signal must exceed bf16 LSB (~0.0625)
...
STUDENT=(
    ...
    # SGD has no buffers (m, v) to offload — disable to skip CPU↔GPU thrashing.
...
echo "============== Update Consistency (BATCH MODE, B=8, 10 steps) =============="

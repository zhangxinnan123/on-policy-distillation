#!/usr/bin/env bash
# Update-consistency — B=1 single-token, lr=1e-2, 50 steps (large-step regime).
# 10x the lr=1e-3 anchor. Purpose: push even more tokens above the bf16 LSB to
# see whether the non-sampled-c sign rule keeps recovering (it climbed to ~78%
# at lr=1e-3 once weak tokens were filtered).
#
# CAVEAT: lr=1e-2 with vanilla SGD is a large step — the per-token linearization
# (Δπ ≈ η·A·shape) may start to break (slope < 1, saturation), and the model may
# drift fast across 50 steps. Read magnitude slope/bias with that in mind; the
# clean perturbative anchor remains lr=1e-3.
#
# Diff vs run_update_consistency_singletoken_b1_lr1e3_50steps_1.7b_8b.sh:
#   only LR bumped from 1e-3 to 1e-2.
...
MEASURE_EVERY=1
OPTIMIZER=SGD
LR=1e-2   # 10x the anchor; large-step regime, linearization may break

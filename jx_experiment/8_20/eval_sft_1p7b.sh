#!/usr/bin/env bash
# Eval-only run using the verl OPD pipeline.
# Loads the SFT'd Qwen3-1.7B checkpoint and runs val_before_train on the test
# split of dapo_17k_aime2426-suffix, no training steps.
#
# Env vars:
#   VAL_MAX_RESPONSE_LENGTH  (default 16384)  # override to 32768 for the long run
#   STUDENT_MODEL            (default = SFT checkpoint dir)
...
FAMILY="Qwen"
STUDENT_MODEL="${STUDENT_MODEL:-/fsx/xinnanzh/checkpoints/sft_qwen3_1p7b_thinking_10k}"
...
# Sampling for val (matches Qwen3 thinking-mode recs)
temperature=1.0
top_p=1.0
top_k=-1
val_top_p=0.95
...
EXP_NAME="eval/${TAG}/val_max${VAL_MAX_RESPONSE_LENGTH}"
...
# distillation.enabled=False → no teacher model spun up
DISTILLATION=(
...
VAL_DUMP_DIR="/fsx/xinnanzh/eval_out/${TAG}/val_max${VAL_MAX_RESPONSE_LENGTH}"

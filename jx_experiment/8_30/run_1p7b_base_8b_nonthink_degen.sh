...
#   save_freq=80, total 200 steps -> saves at 80 / 160 / 200
#   (200 is the last step, and ray_trainer.py:1675 saves on is_last_step too)
#   max_actor_ckpt_to_keep=3      -> keep exactly those 3 (other scripts use 1)
#   save_freq is 80 not 20 on purpose: "keep 3" retains only the MOST RECENT 3,
#   so a denser save_freq would leave 3 checkpoints clustered at the end
#   with no early-training reference. At 80 the 3 survivors span
#   the run, which is what a degeneration progression needs.
#   save_contents=[model,extra]   -> drop optimizer state, ~7GB not ~24GB each
...
EXP_NAME="opd_degen/1p7b_base_8b_nonthink/allpg_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"

...
############################ teacher_topk_entropy τ=1.6 ############################
# Routing (verl/trainer/distillation/hybrid_masks.py:_mask_teacher_topk_entropy):
#   H_topk = -Σ_c p̂(c) · log p̂(c)  over teacher top-K (renormalized)
#   H_topk < τ  → PG (k1)          teacher near-deterministic
#   H_topk ≥ τ  → supervised (FKL) teacher diffuse → dense signal
#
# τ=1.6 nats: for K=16, H_max = log(16) ≈ 2.77 nats. τ=1.6 sits in upper half
# → most tokens still go PG, only high-entropy (diffuse) positions get FKL.
...
EXP_NAME="fsdp/student-${STUDENT_MODEL}/teacher-${TEACHER_MODEL}/loss-${DISTILLATION_LOSS_MODE}/mask-${HYBRID_MASK_STRATEGY}_tau${HYBRID_MASK_TENT_THRESHOLD}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}_fklhybrid"
...
echo "================== teacher_topk_entropy τ=$HYBRID_MASK_TENT_THRESHOLD =================="
echo "DISTILLATION_LOSS_MODE: $DISTILLATION_LOSS_MODE"
echo "HYBRID_MASK_STRATEGY: $HYBRID_MASK_STRATEGY"
echo "HYBRID_MASK_TENT_THRESHOLD: $HYBRID_MASK_TENT_THRESHOLD (nats)"
echo "  H_topk < τ  → PG (k1)     (teacher peaky)"
echo "  H_topk ≥ τ  → FKL (sup)   (teacher diffuse)"

...
############### OPD-theory-guided4 vote=0.5 cov=0.1 eps_low=5 — 1.7B SFT student ###############
# Size-matched partner of the 8B SFT run (job 16033): identical mask settings, so the pair
# isolates student size. Motivation: aopd (pg 67.3) is the only 1.7B result above the noise
# floor (+2.13pp) while being the worst 4B result (-4.00pp), and every high-pg% setting tried
# at 1.7B so far (tent 96.6, opdt4 eps10 95.5) sat inside the floor. eps_low=5 lands between
# those two regimes.
#
...
# Same mask setting as the 8B SFT run (job 16033) so the two are directly comparable across
# student size. eps_low=10 drove pg% to 95.5 here, i.e. it nearly switched the FKL arm off;
# eps_low=5 (ratio > 6) should keep R2 firing. coverage_threshold 0.2 -> 0.1 tightens R1,
# which fired on only 1.07% of positions and added 0.01% beyond R2, so this isolates R2.
...
EXP_NAME="opd/1p7b_400k1ep_teacher_8b/fklhybrid_respool_opdt4_vote${HYBRID_MASK_FKL_VOTE_THRESHOLD}_cov${HYBRID_MASK_COVERAGE_THRESHOLD}_epslow${HYBRID_MASK_EPS_LOW}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"
...
echo "===== 1.7B SFT + opdt4 vote=$HYBRID_MASK_FKL_VOTE_THRESHOLD cov=$HYBRID_MASK_COVERAGE_THRESHOLD eps_low=$HYBRID_MASK_EPS_LOW ====="

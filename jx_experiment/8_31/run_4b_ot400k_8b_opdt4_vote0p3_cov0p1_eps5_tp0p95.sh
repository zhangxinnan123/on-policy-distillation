...
############### OPD-theory-guided4 vote=0.5 cov=0.1 eps_low=5 — 4B ot400k SFT student ###############
# Third member of the size-matched set with jobs 16033 (8B SFT) and 16034 (1.7B SFT): identical
# mask settings across all three students, so the trio isolates student size. 4B is the size
# that needs explaining -- aopd is +2.13pp at 1.7B and -4.00pp here at nearly the same routing
# fraction (67.3% vs 70.6% pg), so 4B is the only negative aopd run among the SFT students.
#
...
FAMILY="Qwen"
STUDENT_MODEL=/fsx/xinnanzh/checkpoints/sft_qwen3_4b_ot400k
...
# Halved to match SP=2. The previous 4B runs left this at the full 18432 while still setting
# SP=2, which throws away the point of Ulysses: each rank then packs up to 18432 tokens of
# activations instead of 9216. Job 16035 died in loss.backward() at step 59 needing 12.32 GiB
# with 11.98 GiB free -- a 0.34 GiB shortfall on a margin that was always this thin (its FKL
# share was only 1.4%, the lowest of the three 4B runs, so routing was NOT the cause).
...
EXP_NAME="opd/4b_ot400k_teacher_8b/fklhybrid_opdt4_vote${HYBRID_MASK_FKL_VOTE_THRESHOLD}_cov${HYBRID_MASK_COVERAGE_THRESHOLD}_epslow${HYBRID_MASK_EPS_LOW}_tp${HYBRID_MASK_TEACHER_TOP_P}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"
...
echo "===== 4B SFT + opdt4 vote=$HYBRID_MASK_FKL_VOTE_THRESHOLD cov=$HYBRID_MASK_COVERAGE_THRESHOLD eps_low=$HYBRID_MASK_EPS_LOW top_p=$HYBRID_MASK_TEACHER_TOP_P ====="

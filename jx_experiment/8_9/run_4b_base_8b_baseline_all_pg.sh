...
############################ Baseline: all PG (no routing) ############################
# Compare against teacher_topk_entropy τ=1.6 (companion script).
# hybrid_mask_strategy="all" → every valid response token goes to PG arm (k1),
# equivalent to pure on-policy REINFORCE-RKL. No supervised (FKL) contribution.
...
HYBRID_MASK_STRATEGY="all"      # all tokens → PG (baseline)
...
EXP_NAME="fsdp/student-${STUDENT_MODEL}/teacher-${TEACHER_MODEL}/loss-${DISTILLATION_LOSS_MODE}/mask-${HYBRID_MASK_STRATEGY}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}_baseline"
...
echo "================== Baseline (all-PG) =================="
echo "DISTILLATION_LOSS_MODE: $DISTILLATION_LOSS_MODE"
echo "HYBRID_MASK_STRATEGY: $HYBRID_MASK_STRATEGY (baseline — every token → PG)"

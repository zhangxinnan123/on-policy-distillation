...
############################ Degeneration demo — 1.7B-Base student, 8B non-think teacher ############################
# Purpose: produce SAVED CHECKPOINTS across training so the progressive
# degeneration of generations under pure all-token REINFORCE-RKL can be
# visualized. Not a benchmark run.
#
# Configuration is deliberately the *degenerating* one:
#   loss = k1_topk_overlap, use_policy_gradient=True, mask="all"
#   -> every response token gets the single-sample RKL PG update, no FKL arm.
# (Under k1_topk_overlap the mask is a diagnostic only — see losses.py:1138 and
#  _compute_pg_diag_for_non_hybrid — so "all" is stated for clarity, not effect.)
#
# Non-think: +data.apply_chat_template_kwargs.enable_thinking=False
# substitute_eos_token=True because the student is a *base* model using
# <|endoftext|> while the teacher is instruct using <|im_end|>. Without it the
# teacher scores the wrong EOS at sequence end, which produces garbage for
# reasons unrelated to the algorithm and would confound the demo.
#
# Checkpointing (the point of this script):
#   save_freq=-1 -> no checkpoints written
#   (200 is the last step, and ray_trainer.py:1675 saves on is_last_step too)
#   with no early-training reference. At 80 the 3 survivors span
#   the run, which is what a degeneration progression needs.
#   test_freq=20 + log_val_generations=20 -> val samples in wandb every 20 steps
...
FAMILY="Qwen"
# Pure forward-KL over EVERY token (loss_mode=forward_kl_topk, non-hybrid).
# This is the 100%-FKL endpoint of the routing axis. The 0% endpoint at this student is
# the all-PG degeneration reference (1.7B-Base n1mxr8w7, avg4 14.35). Every mask studied
# so far sits between them at 0.6%-33% FKL with no monotone trend, so the endpoints are
# what the axis is missing.
...
USE_POLICY_GRADIENT=False  # forward_kl_topk 不读它；显式置 False 以免误读脚本
# Loss combine: RKL (k1 PG) over ALL response tokens + FKL-topk supervised
# restricted to the mask-selected subset. The opd_theory_guided mask still
# decides which tokens additionally get FKL.
...
DISTILLATION_LOSS_MODE="forward_kl_topk"   # 非 hybrid: 全 token 走 FKL，不经过 mask
USE_FUSED_KERNELS=False

# Theory-guided OPD masking strategy.
# See verl/trainer/distillation/hybrid_masks.py:_mask_opd_theory_guided for the
# full routing rules (low-coverage, negative-k1, positive-k1).
HYBRID_MASK_STRATEGY="all"      # all tokens -> PG
HYBRID_MASK_STUDENT_LOW_THRESHOLD=5e-2   # Rule 1: pi_c < this  → FKL
HYBRID_MASK_STUDENT_HIGH_THRESHOLD=$(python3 -c "print(1 - $HYBRID_MASK_STUDENT_LOW_THRESHOLD)")  # Rule 1b: pi_c > this & teacher wants ↓ → FKL
HYBRID_MASK_TEACHER_TOP_P=0.95            # nucleus mass for teacher candidates c
...
HYBRID_MASK_USE_NEGATIVE_RULE=True
HYBRID_MASK_USE_POSITIVE_RULE=True
...
# Halved to match SP=2. forward_kl_topk routes 100% of tokens through the top-k FKL kernel,
# which is the largest (tokens, vocab) peak of any setting run so far -- aopd only sent 24-33%
# there. Leaving this at the full 18432 while setting SP=2 is what killed jobs 16035 and
# xglsupnb. seqlen_balancing.py:384 still holds: 3072 * SP(2) = 6144 >= max_seq_len 6144.
...
SP=2   # ulysses: 全 token FKL 时 (tokens,vocab) logits 峰值最大

ROLLOUT_N=1
LR=1e-6
train_batch_size=$(( TRAIN_PROMPT_BSZ ))

EXP_NAME="opd_fklall/1p7b_base_8b_nonthink/fullfkl_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"
CKPT_DIR=/fsx/xinnanzh/ckpts/opd_degen_1p7b_base_8b_nonthink
...
DISTILLATION=(
    ...
    +distillation.distillation_loss.hybrid_mask_kwargs.student_low_threshold=$HYBRID_MASK_STUDENT_LOW_THRESHOLD
    +distillation.distillation_loss.hybrid_mask_kwargs.student_high_threshold=$HYBRID_MASK_STUDENT_HIGH_THRESHOLD
...
echo "================== Theory-Guided OPD Mask Configuration =================="
echo "DISTILLATION_LOSS_MODE: $DISTILLATION_LOSS_MODE"
echo "HYBRID_MASK_STRATEGY: $HYBRID_MASK_STRATEGY"
echo "HYBRID_MASK_STUDENT_LOW_THRESHOLD: $HYBRID_MASK_STUDENT_LOW_THRESHOLD"
echo "HYBRID_MASK_STUDENT_HIGH_THRESHOLD: $HYBRID_MASK_STUDENT_HIGH_THRESHOLD"
echo "HYBRID_MASK_TEACHER_TOP_P: $HYBRID_MASK_TEACHER_TOP_P"
echo "HYBRID_MASK_ADV_EPS: $HYBRID_MASK_ADV_EPS"
...
echo "HYBRID_MASK_USE_NEGATIVE_RULE: $HYBRID_MASK_USE_NEGATIVE_RULE"

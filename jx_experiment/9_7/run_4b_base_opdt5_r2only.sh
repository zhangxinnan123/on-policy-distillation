...
########## opd_theory_guided5 ablation arm: R2 only: under-coverage vote, R3 off ##########
# Three arms per student, R1 off in all of them, everything else identical:
#   r2only  under-coverage vote only        (this file's sibling)
#   r3only  over-confidence ANY+floor only   (this file's sibling)
#   r2r3    both                             (jobs 16324/16325)
# R2-only here is NOT the same as the v4 r2only arm: that ran eps_low=0.5,
# i.e. ratio 1.5, whereas this is ratio_low=10.
...
EXP_NAME="opd_ablate/4b_base_8b/opdt5_r2only_rlow${HYBRID_MASK_RATIO_LOW}_rhigh${HYBRID_MASK_RATIO_HIGH}_floor${HYBRID_MASK_OVERSHOOT_FLOOR}_vote${HYBRID_MASK_FKL_VOTE_THRESHOLD}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"
...
DISTILLATION=(
    ...
    +distillation.distillation_loss.hybrid_mask_kwargs.use_overconfident_rule=False

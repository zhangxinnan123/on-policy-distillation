...
#   R2 (M3 vote):  teacher-prob-weighted vote of [pi_T(c)/pi_S(c) > 1+eps_low] > vote -> FKL
...
# This arm: R1 only (coverage rule), R2 disabled
...
HYBRID_MASK_USE_COVERAGE_RULE=True      # R1
...
EXP_NAME="opd_ablate/8b_base_8b/opdt4_r1only_vote${HYBRID_MASK_FKL_VOTE_THRESHOLD}_cov${HYBRID_MASK_COVERAGE_THRESHOLD}_eps${HYBRID_MASK_EPS_LOW}_b${train_batch_size}_n${ROLLOUT_N}_lr${LR}_reslen${MAX_RESPONSE_LENGTH}"

#!/usr/bin/env bash
# SDPO+GRPO on deepmath, dual-KL recipe:
#   reward      = outcome - β_init · KL(π_cur(y|x) ‖ π_init(y|x))
#                                            (origin KL via use_kl_in_reward=True)
#   advantage   = GRPO(reward) reweighted by SDPO sign-flip
#                 (hint KL effect via compute_sdpo_grpo_advantage)
#
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_sdpo_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_lneg${SDPO_SIGN_FLIP_LAMBDA_NEG}_kl${ORIGIN_KL_COEF}_${DATE_TAG}}
...
SDPO=(
    ...
    # Hint rollouts run for DIAGNOSTIC + SOLVABLE LABELING only; do NOT merge
    # them into the main batch. compute_sdpo_grpo_advantage then takes the
    # legacy path (is_hint_row=None): vanilla GRPO over n main rows per uid,
    # sign-flip applied only on prompts whose hint_solvable=True. No regime
    # A/B/C dispatch, no group=n+n_hint expansion.

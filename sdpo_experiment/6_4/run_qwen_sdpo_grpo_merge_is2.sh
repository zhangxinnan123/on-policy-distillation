#!/usr/bin/env bash
# SDPO+GRPO on deepmath, MERGE + STRICT-UNBIASED IS CORRECTION. No origin KL.
#
...
# When ON: hint rows are swapped to un-hinted [x] before the actor update so the
# PPO ratio becomes the IS weight π(y|x)/π(y|x,e), bounded by actor.clip_ratio.
# REQUIRES use_rollout_log_probs=True (else the on-policy detach zeros the IS).
IS_CORRECTION=${IS_CORRECTION:-True}

# Wider upper PPO clip so hint rows whose IS weight π(y|x)/π(y|x,e) > 1 (student
# already likes y_hint without the hint) aren't capped as aggressively. Only the
# HIGH side matters here; the low side doesn't floor small IS weights for A>0.
...
STUDENT=(
    ...
    # Wider upper clip for the IS ratio on hint rows.
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_sdpo_merge_is${IS_CORRECTION}_n${N_HINT}_cliplo${CLIP_RATIO_LOW}_cliphi${CLIP_RATIO_HIGH}_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_lneg${SDPO_SIGN_FLIP_LAMBDA_NEG}_noklbase_${DATE_TAG}}

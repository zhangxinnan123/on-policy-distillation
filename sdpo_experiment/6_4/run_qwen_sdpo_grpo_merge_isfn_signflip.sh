#!/usr/bin/env bash
# SDPO+GRPO on deepmath, MERGE + IS-f(w) (regime C) + SELF-KL sign-flip (regime B).
# Combines both hint mechanisms, acting on disjoint regimes so they compose:
#   regime B (hint+main both succeed): main-row advantage reweighted by SELF-KL
#       sign-flip, delta = log π_θ(y|x,e) - log π_θ(y|x). λ = SDPO_SIGN_FLIP_*.
#   regime C (stuck: main all fail, hint solves): hint rows enter the group;
#       their IS weight w = π(y|x)/π(y|x,e) is squashed by f(w)=w/(w+IS_FN_CONST)
#       and applied as rollout_is_weights (biased-but-tunable rescue).
# No origin KL (pure outcome reward).
#
...
# script combines it with the regime-C IS-f(w) rescue. Acts on disjoint regimes
# from the IS correction, so the two compose without double-counting.
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_sdpo_merge_isfn${IS_FN_CONST}_signflip_n${N_HINT}_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_lneg${SDPO_SIGN_FLIP_LAMBDA_NEG}_eps${SDPO_SIGN_FLIP_EPSILON}_noklbase_${DATE_TAG}}

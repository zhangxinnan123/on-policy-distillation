#!/usr/bin/env bash
# LUFFY-style gradient AMPLIFICATION on positive-advantage hint tokens.
#
# Per-token amplifier applied to PPO min-clip surrogate:
#     amplifier(p) = 4·γ² / (p + γ)²
#                    where p = π_θ(y_hint | x)  (deployment prob, raw)
# Applied ONLY where is_hint_row=True AND advantage>0. Other tokens keep 1.0.
#
# amplifier > 1 for p < γ   (boost rare hint tokens)
# amplifier = 1 at p = γ    (inflection)
# amplifier < 1 for p > γ   (dampen common tokens)
#
# Compatible with PPO min-clip via use_rollout_log_probs=True: amplifier
# multiplies onto the clipped surrogate, so extreme IS ratios still trigger
# PPO clip's gradient stop.
set -xeuo pipefail
...
# Sign-flip OFF by default (isolate the amplifier effect).
...
############################ Amplifier ############################
# γ = inflection point. amplifier = 1 at p=γ, > 1 for p<γ, < 1 for p>>γ.
...
STUDENT=(
    ...
    # amplify mode requires TRUE PPO ratio so the amplifier multiplies onto the
    # PPO-clipped surrogate (not a fake ratio=1).
...
SDPO=(
    ...
    # Amplify mode: gradient modifier 4γ²/(p+γ)² on positive-adv hint tokens.
    +sdpo.sdpo_grpo.is_correction.enabled=True
    +sdpo.sdpo_grpo.is_correction.fn_const=0.0
    +sdpo.sdpo_grpo.is_correction.mode=amplify_linear
    +sdpo.sdpo_grpo.is_correction.gamma=${AMPLIFY_GAMMA}

    # Regime C uses fixed A=+1 (hint) / A=0 (main), no GRPO normalization,
    # no punishment. regime_c_scale multiplies onto the +1 (default 1.0).
    +sdpo.sdpo_grpo.regime_c_scale=${REGIME_C_SCALE:-1.0}
)

EXP_NAME=${EXP_NAME:-qwen_deepmath_amplifyLinear_gamma${AMPLIFY_GAMMA}_ppoMinClip_epsLo${PPO_CLIP_LOW}_epsHi${PPO_CLIP_HIGH}_fixedRegC${REGIME_C_SCALE:-1.0}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

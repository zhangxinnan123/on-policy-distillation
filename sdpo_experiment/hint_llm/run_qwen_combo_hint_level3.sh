#!/usr/bin/env bash
# COMBINATION: Sign-flip (Regime B) + LUFFY (Regime C hint reshape).
#
# Purpose: measure whether SDPO's sign-flip advantage manipulation and LUFFY's
# f(p) reshape on hint gradients are additive (or interfere) in improving math
# reasoning on deepmath_diff6to8.
#
# Effective per-token pg_loss:
#   Main tokens (all regimes):
#     -A_main × ratio × 1
#     where A_main in Regime B has sign-flip scale (1-λ)+λ·exp(sign(A)·delta).
#   Hint tokens (only Regime C has A≠0):
#     -A_hint × ratio × fp
#     fp = π_θ(y|x)/(π_θ(y|x) + c),  ratio=1 when use_rollout_log_probs=False.
#
# Regime dispatch:
#   A: hint fails         → GRPO on main only; no sign-flip; hint adv=0
#   B: both succeed       → GRPO(main) with sign-flip reweight; hint adv=0
#   C: main all fail      → GRPO on main+hint group; hint has adv, reshaped by fp
#
# Compared to run_qwen_luffy_hint_level3.sh (LUFFY-only):
#   sign_flip_lambda_{pos,neg}=0.3      ← Regime B sign-flip ON
#   (LUFFY p_div_p_c and merge/is_correction unchanged)
set -xeuo pipefail
...
# Same headroom-safe token budget as the LUFFY-only script.
...
# Regime B sign-flip: ON. Default λ=0.3, override via env if needed.
...
SDPO=(
    ...
    # Sign-flip ON (Regime B main-row advantage reweight).
    ...
    # LUFFY p/(p+c) reshape ON (Regime C hint-row gradient reshape).
    +sdpo.sdpo_grpo.is_correction.enabled=True
    +sdpo.sdpo_grpo.is_correction.fn_const=${LUFFY_C}

    # Regime C advantage scale α: dampens |advantage| inflation from tiny group std.
    +sdpo.sdpo_grpo.regime_c_scale=${REGIME_C_SCALE:-0.25}
)

EXP_NAME=${EXP_NAME:-qwen_deepmath_signFlip_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_luffyShape_c${LUFFY_C}_regCscale${REGIME_C_SCALE:-0.25}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

#!/usr/bin/env bash
# FULL SDPO + PPO min-clip IS: sign-flip on Regime B, PPO-clip IS on Regime C hint.
#
# Combo of the two mechanisms:
#   1. SDPO sign-flip (Regime B): reweight main-row advantage by
#      (1-λ) + λ · exp(sign(A) · [log π(y|x,e) − log π(y|x)])
#   2. PPO min-clip on hint rows (Regime C): pg_loss = max(-A·w, -A·clip(w,0.8,1.28))
#      where w = π_θ(y|x) / π_θ_old(y|x, e) is the true IS ratio.
#
# Config anchors:
#   sign_flip_lambda_{pos,neg}=0.1              (Regime B ON)
#   is_correction.enabled=True, fn_const=0.0    (implicit-ratio path)
#   use_rollout_log_probs=True                  (PPO ratio = IS ratio for hint)
#   actor.clip_ratio_low=0.2, clip_ratio_high=0.28  (DAPO asymmetric)
#   regime_c_scale=0.25                         (Regime C α damper)
set -xeuo pipefail
...
# Regime B sign-flip: ON. Default λ=0.1.
...
STUDENT=(
    ...
    # Keep stored old_log_probs so PPO ratio for hint rows = IS ratio.
...
SDPO=(
    ...
    # Sign-flip ON (Regime B main-row reweight).
    ...
    # Implicit IS mode: hint rows swap, PPO ratio becomes IS ratio, PPO clip fires.
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_signFlip_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_ppoMinClip_epsLo${PPO_CLIP_LOW}_epsHi${PPO_CLIP_HIGH}_regCscale${REGIME_C_SCALE:-0.25}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

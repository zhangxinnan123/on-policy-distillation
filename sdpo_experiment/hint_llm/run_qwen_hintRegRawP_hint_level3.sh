...
SDPO=(
    ...
    # input_type = "raw_p" — f(p) where p = π_θ(y|x), no denominator.
    # Matches LUFFY paper exactly (vs "ratio" which uses IS ratio).
    +sdpo.hint_reg.input_type=raw_p
)

EXP_NAME=${EXP_NAME:-qwen_deepmath_hintRegRawP_gamma${HINT_REG_GAMMA}_coef${HINT_REG_COEF}_ppoMinClip_epsLo${PPO_CLIP_LOW}_epsHi${PPO_CLIP_HIGH}_fixedRegC${REGIME_C_SCALE:-1.0}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

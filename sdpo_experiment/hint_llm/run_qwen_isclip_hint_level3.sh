#!/usr/bin/env bash
# HARD-CLIP IS correction only — no LUFFY shaping, no sign-flip.
#
# Hint row per-token weight = clip(w, clip_low, clip_high) where
#     w = π_θ(y|x) / π_θ(y|x, e).
# Standard PPO-style importance sampling with bounded ratio. This is Setting A
# in the "IS clip vs LUFFY shaping" comparison. Setting B = run_qwen_luffy_*.sh.
#
# Config:
#   +sdpo.sdpo_grpo.is_correction.mode=clip     (vs default "shaping")
#   +sdpo.sdpo_grpo.is_correction.fn_const=0.0  (shaping OFF)
#   +sdpo.sdpo_grpo.is_correction.clip_low=0.1
#   +sdpo.sdpo_grpo.is_correction.clip_high=10.0
#   +sdpo.sdpo_grpo.sign_flip_lambda_*=0.0      (sign-flip OFF)
#   +sdpo.sdpo_grpo.regime_c_scale=0.25         (advantage dampener)
set -xeuo pipefail
...
# Sign-flip HARD-OFF (this is the clip-only baseline).
SDPO_SIGN_FLIP_LAMBDA_POS=0.0
SDPO_SIGN_FLIP_LAMBDA_NEG=0.0
SDPO_SIGN_FLIP_EPSILON=0.2

############################ IS clip bounds ############################
# DAPO asymmetric PPO clip: [1-ε_low, 1+ε_high] = [0.8, 1.28] (ε_low=0.2, ε_high=0.28).
IS_CLIP_LOW=${IS_CLIP_LOW:-0.8}
IS_CLIP_HIGH=${IS_CLIP_HIGH:-1.28}
...
STUDENT=(
    ...
    # Explicit IS-clip needs NEUTRAL PPO ratio so coefficient = clip(w), not clip(w)·ratio.
...
SDPO=(
    ...
    # Hard IS-clip on hint rows (no LUFFY shaping).
    +sdpo.sdpo_grpo.is_correction.enabled=True
    +sdpo.sdpo_grpo.is_correction.mode=clip
    +sdpo.sdpo_grpo.is_correction.fn_const=0.0
    +sdpo.sdpo_grpo.is_correction.clip_low=${IS_CLIP_LOW}
    +sdpo.sdpo_grpo.is_correction.clip_high=${IS_CLIP_HIGH}

    # Regime C advantage scale (same as luffy script).
    +sdpo.sdpo_grpo.regime_c_scale=${REGIME_C_SCALE:-0.25}
)

EXP_NAME=${EXP_NAME:-qwen_deepmath_hardClip_lo${IS_CLIP_LOW}_hi${IS_CLIP_HIGH}_regCscale${REGIME_C_SCALE:-0.25}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

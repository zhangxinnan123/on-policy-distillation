#!/usr/bin/env bash
# STANDARD PPO min-clip form on the IS ratio for hint rows.
#
# Uses the trainer's implicit-ratio path (existing) with DAPO clip bounds:
#   pg_loss[hint] = max(-A·w, -A·clip(w, 1-0.2, 1+0.28))
#                 = max(-A·w, -A·clip(w, 0.8, 1.28))
# where w = π_θ(y_hint|x) / π_θ_old(y_hint|x, e).
#
# Main rows keep the standard PPO on-policy ratio ≈ 1 (clip is no-op).
#
# Diff vs LUFFY shaping (run_qwen_luffy_hint_level3.sh):
#   fn_const=0.0                        (LUFFY shaping OFF)
#   use_rollout_log_probs=True          (so PPO ratio = true IS ratio w, not 1)
#   actor.clip_ratio_low=0.2            (PPO clip low, DAPO)
#   actor.clip_ratio_high=0.28          (PPO clip high, DAPO asymmetric)
#
# Diff vs run_qwen_isclip_hint_level3.sh (mode=clip):
#   That version does `-A · clip(w)` (single term, no min).
#   THIS version does `max(-A·w, -A·clip(w))` — proper PPO pessimistic bound.
set -xeuo pipefail
...
# clip_ratio_low = ε_low: PPO ratio floor = 1 - ε_low = 0.8
# clip_ratio_high = ε_high: PPO ratio ceiling = 1 + ε_high = 1.28
...
STUDENT=(
    ...
    # KEY: keep stored old_log_probs (hinted logprob for hint rows) so PPO ratio
    # for hint rows = π_θ(y|x)/π_θ_old(y|x,e) = the true IS ratio.
    +actor_rollout_ref.actor.use_rollout_log_probs=True
    # DAPO asymmetric PPO clip [1-0.2, 1+0.28] = [0.8, 1.28].
...
SDPO=(
    ...
    # Implicit IS mode: swap hint rows to unhinted prompt, let PPO ratio be
    # the IS weight, standard PPO min-clip handles bounding.
    +sdpo.sdpo_grpo.is_correction.enabled=True
    +sdpo.sdpo_grpo.is_correction.fn_const=0.0
    # (mode defaults to "shaping" but fn_const=0 skips the explicit branch;
    #  implicit path is taken.)

    +sdpo.sdpo_grpo.regime_c_scale=${REGIME_C_SCALE:-0.25}
)

EXP_NAME=${EXP_NAME:-qwen_deepmath_ppoMinClip_epsLo${PPO_CLIP_LOW}_epsHi${PPO_CLIP_HIGH}_regCscale${REGIME_C_SCALE:-0.25}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

#!/usr/bin/env bash
# SDPO with sign-flip DECAY (0.1 → 0.001) + PPO min-clip IS on Regime C hint.
#
# Combines:
#   - Sign-flip on Regime B main rows, cosine-decaying λ from 0.1 to 0.001.
#     Late training gets almost no sign-flip pressure (prevents entropy
#     over-shoot seen in 9187 combo).
#   - PPO min-clip IS on Regime C hint rows (regime_c_scale=0.25).
#   - NO always_merge — hint only enters loss when main is fully stuck
#     (avoids the entropy collapse of 9232).
set -xeuo pipefail
...
# Sign-flip: base λ=0.1, decay to 0.001 (i.e. final_scale = 0.001/0.1 = 0.01).
...
SDPO=(
    ...
    # Merge hint only when Regime C fires (main all fail + hint succ).
    +sdpo.hint_rollout.merge_into_group=True
    # Explicitly OFF: hint stays out of Regime B group (avoid entropy collapse).
    +sdpo.sdpo_grpo.always_merge_hint_into_group=False

    # Sign-flip ON at base λ, decayed to 0.001 (final_scale=0.01 relative).
    ...
    # Implicit IS: PPO ratio auto = IS ratio on hint rows, DAPO min-clip fires.
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_sdpoDecay_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}to${SDPO_DECAY_FINAL_SCALE}_ppoMinClip_epsLo${PPO_CLIP_LOW}_epsHi${PPO_CLIP_HIGH}_regCscale${REGIME_C_SCALE:-0.25}_n${N_HINT}_hint${HINT_LEVEL}_${DATE_TAG}}

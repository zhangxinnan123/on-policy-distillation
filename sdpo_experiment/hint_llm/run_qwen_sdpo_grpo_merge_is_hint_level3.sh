#!/usr/bin/env bash
# SDPO+GRPO on deepmath with LLM-EXTRACTED HINTS (Claude Haiku 4.5, level_3).
#
# Same wiring as run_qwen_sdpo_grpo_merge_is.sh (MERGE + STRICT-UNBIASED IS),
# except:
#   - Training dataset is train_with_hints.parquet (adds level_1/2/3 columns).
#   - Expert e is the 3-level hint at level_3 (~83 words, detailed setup) instead
#     of the full teacher solution in generations_wo_think.
#
# Toggle to a different hint level via env:
#     HINT_LEVEL=level_2 bash ...
set -xeuo pipefail
...
# NEW: parquet includes level_1 / level_2 / level_3 columns from Claude hints.
...
# NEW: point at one of level_1 / level_2 / level_3. Default is level_3 (most
# detailed hint). Column values are plain strings; _get_expert_texts._pick
# handles str vs list transparently, and expert_index is a no-op for strings.
...
EXP_NAME=${EXP_NAME:-qwen_deepmath_sdpo_merge_is${IS_CORRECTION}_n${N_HINT}_hint${HINT_LEVEL}_lpos${SDPO_SIGN_FLIP_LAMBDA_POS}_${DATE_TAG}}

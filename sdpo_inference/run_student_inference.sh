...
DATASET_PATH=LlamaFactory/data/deepmath_diff6to8_verified.parquet
OUT_DIR="/projects/standard/mhong/zhan9359/work/on-policy-distillation/verl/sdpo_result/Qwen3-4B"
...
python sdpo_inference/run_generate_parquet_with_logpi.py \
  --parquet "${DATASET_PATH}" \
  --model $model \
  --out_dir "${OUT_DIR}" \
  --max_model_len "${MAX_LEN}" \

...
OUT_DIR="/projects/standard/mhong/zhan9359/work/on-policy-distillation/verl/lzy_eval_result/Qwen3-8B"
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python run_generate_parquet.py \
  --parquet "${DATASET_PATH}" \
  --model  $model \
  --out_dir "${OUT_DIR}" \
  --max_model_len "${MAX_LEN}" \
